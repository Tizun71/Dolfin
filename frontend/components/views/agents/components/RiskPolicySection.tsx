...
export default function RiskPolicySection({
  ...
  const presetCls = (active: boolean) =>
    `flex-1 min-w-[120px] text-left p-4 border transition ${
      active ? "border-[#fb923c] bg-[#fb923c12]" : "border-[#222] hover:border-[#333]"
    }`;
  const chip = (active: boolean) =>
    `px-3 py-2 border text-xs font-mono transition ${
      active ? "border-[#fb923c] bg-[#fb923c1a] text-[#fbbf24]" : "border-[#222] text-[#666] hover:border-[#333]"
    }`;
          ...
          <div className="grid grid-cols-2 gap-4">
            {USD_FIELDS.map((f) => (
              <div key={f.key as string}>
                ...
                <div className="flex items-center border border-[#222] bg-[#050505] focus-within:border-[#fb923c] transition">
