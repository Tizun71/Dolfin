...
/** Aave action names the strategy layer may emit (mirror of the policy mask). */
...
/**
 * Build a policy-shaped Aave TradeDecision. protocol/adapter come from config (never
 * from a caller or the LLM), so an AI proposal can only pick action + token + size —
 * it can't inject an arbitrary pool or adapter address.
 */
...
/** Resolve a token symbol against the agent's registry; undefined if not whitelisted. */
export function resolveToken(symbol: string, cfg: OnchainConfig): TokenInfo | undefined {
  return cfg.tokens[symbol.toUpperCase()];
}

/** Map an Aave action name to its ActionType enum; undefined if unknown. */
